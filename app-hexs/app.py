...
def filter_points(points, kernel_size=3, target_resolution=50):
    if not points:
        return []
    ...
    if lng_range >= lat_range:
        ...
        height = max(1, round(target_resolution * (lat_range / (lng_range + 1e-12))))
    else:
        height = target_resolution
        width = max(1, round(target_resolution * (lng_range / (lat_range + 1e-12))))
    mask = np.zeros((height, width), dtype=np.uint8)
    for lat, lng, _ in points:
        x = int((lng - lng_min) / (lng_range + 1e-12) * (width - 1))
        y = int((lat_max - lat) / (lat_range + 1e-12) * (height - 1))
        mask[y, x] = 1
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        # Invert colormap button full width
        html.Div(
        ...
        html.Div([
            ...
            html.Div(
                ...
                style={"width": "78%", "max-width": "150px", "margin-top": "2px"}
            )
        ], className="panel-section",
            style={"display": "flex", "align-items": "center", "gap": "10px", "margin-bottom": "2px"}),
        ...
        # Resolution buttons row (Low/Medium/High)
        html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div("Grid size:", style={"margin-bottom": "10px","font-weight": "bold","font-family": "Roboto, sans-serif","font-size": "14px"}),
                html.Div(
                    dcc.Slider(
                        id="grid-size-slider",
                        min=20,
                        ...
                        step=10,
                        value=50,
                        marks={i: str(i) for i in range(20, 501, 80)},
                        tooltip={"placement": "bottom", "always_visible": False},
                    ),
                    style={"width": "100%"}
                )
...
@app.callback(
    ...
    State("grid-size-slider", "value"),
    prevent_initial_call=True
)
def download_hex_layer(n_clicks, counter, zoom, agg_method, threshold,
                       scale_mode, fixed_res, zoom_profile,
                       filter_value, kernel_size, grid_size):
    counter += 1  # zwiększamy licznik pobrań
    zoom = zoom or 7
    show_res = fixed_res if scale_mode == "fixed" else zoom_to_resolution(zoom, zoom_profile)

    if "filter" in filter_value:
        filtered_points = filter_points(points, kernel_size=kernel_size, target_resolution=grid_size)
        cell_values = {}
...
def update_layer(
    ...
    fixed_res, zoom_profile, filter_value, kernel_size, grid_size, opacity
):
    zoom = zoom or 7
    show_res = fixed_res if scale_mode == "fixed" else zoom_to_resolution(zoom, zoom_profile)
    invert = n_clicks % 2 == 1
